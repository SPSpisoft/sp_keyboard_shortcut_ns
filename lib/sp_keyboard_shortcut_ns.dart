library sp_keyboard_shortcut_ns;
...
void initShortCuts(
    Widget homePage, {
      required Set<Set<LogicalKeyboardKey>> keysToPress,
      required Set<Function(BuildContext context)> onKeysPressed,
...
bool _isPressed(Set<LogicalKeyboardKey> keysPressed, Set<LogicalKeyboardKey>? keysToPress) {
  ...
  for (final rightElement in rights) {
    var leftElement =
    lefts.firstWhere((element) => element.debugName!.split(" ")[0] == rightElement.debugName!.split(" ")[0]);
    var actualKey = keysToPress!.where((element) => element.debugName!.split(" ")[0] == rightElement.debugName!.split(" ")[0]);
    if (actualKey.isNotEmpty && actualKey.first.debugName!.isNotEmpty) {
...
class _KeyBoardShortcuts extends State<KeyBoardShortcuts> {
  ...
  void listener(RawKeyEvent v) async {
    ...
    if (v.runtimeType == RawKeyDownEvent) {
      ...
      if (widget.onKeysPressed !=null && _isPressed(keysPressed, widget.keysToPress)) {
                      ...
                      _customGlobal ?? ListBody(
                        children: [
                          ...
                          ListTile(
                            ...
                            subtitle: Text(LogicalKeyboardKey.home.debugName?? "HOME"),
                          ),
                          ListTile(
                            leading: const Icon(Icons.subdirectory_arrow_left),
                            title: const Text("Go on last page"),
                            subtitle: Text(LogicalKeyboardKey.escape.debugName?? "ESC"),
                          ),
                          ListTile(
                            leading: const Icon(Icons.keyboard_arrow_up),
                            title: const Text("Scroll to top"),
                            subtitle: Text(LogicalKeyboardKey.pageUp.debugName?? "UP"),
                          ),
                          ListTile(
                            leading: const Icon(Icons.keyboard_arrow_down),
                            title: const Text("Scroll to bottom"),
                            subtitle: Text(LogicalKeyboardKey.pageDown.debugName?? "DOWN"),
                          ),
                        ],
                      ),
                  ],
...
Widget _helpWidget(_KeyBoardShortcuts w) {
  String text = _getKeysToPress(w.widget.keysToPress);
  if (text != "") {
    return ListTile(
      leading: Icon(_customIcon ?? Icons.settings),
      title: Text(w.widget.helpLabel?? ""),
      subtitle: Text(text),
    );
  }
  return Container();
